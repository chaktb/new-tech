...
def filter_items(items, seen_links=None, seen_titles_prev=None):
    ...
    for it in items:
        ...
        seen_titles.add(tk)
        kept.append(it)
    log(f"  필터 내역: 전체 {len(items)} / 시간창밖 {n_old} / 키워드불일치 {n_nokw} "
        f"/ 이미게시·중복 {n_dup} / 통과 {len(kept)}")
    kept.sort(key=lambda x: x["published"], reverse=True)
    if len(kept) > MAX_ITEMS:
        log(f"  상위 {MAX_ITEMS}건만 게시 (나머지 {len(kept)-MAX_ITEMS}건은 다음 회차 후보)")
